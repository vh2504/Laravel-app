            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h3 class="mb-0">{{ __('feature.update') }}</h3>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form action="{{route('admin.features.update',$feature->id)}}" method="POST" id="form-feature" enctype='multipart/form-data'>
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="name" type="text" value="{{$feature->name}}">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control{{ $errors->has('type_group') ? ' is-invalid' : '' }}" name="type_group" id="type_group" data-toggle="select">
                                        ...
                                        <option @if($feature->type_group->value==$group->value) selected @endif value="{{$group->value}}">{{__('feature.type_group.'.strtolower($group->name))}}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        @php
                        $listJobCategoryId = $feature->jobCategories->pluck('id')->toArray();
                        @endphp
                        <div class="accordion" id="accordionFeature">
                            @foreach($collections as $index => $collection)
                            <div class="card">
                                <div class="card-header" id="heading-{{$index + 1}}" data-toggle="collapse" data-target="#collapse-{{$index + 1}}" aria-expanded="false" aria-controls="collapse-{{$index + 1}}">
                                    <h5 class="mb-0"> {{$index + 1}}. {{$collection->name}}:</h5>
                                </div>
                                <div id="collapse-{{$index + 1}}" class="collapse" aria-labelledby="heading-{{$index}}" data-parent="#accordionFeature">
                                    <div class="card-body">
                                        <div class="row px-3">
