    ...
    <div class="row">
        <div class="col">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('admin.posts.update', $post->id)}}" method="POST" id="form-post" enctype='multipart/form-data'>
                        @csrf
                        <input name="_method" type="hidden" value="PUT">
                        <div class="form-group">
                            <label class="form-control-label">{{__('post.create-page.image')}}</label>
                            <div class="dropzone dropzone-single mb-3 dz-started dz-max-files-reached " id="my-dropzone" style="width:500px">
                                <div class="fallback">
                                ...
                                <div class="dz-preview-single">
                                    <div class="dz-preview-cover dz-processing dz-image-preview">
                                        <img class="dz-preview-img" src="{{$post->image}}">
                                    </div>
                                </div>
                        ...
                        <input type="hidden" name="image" value="{{$post->image}}" class="custom-file-input" id="image">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-control-label" for="title">{{__('post.create-page.title')}}</label>
                                    <input class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" name="title" id="title" type="text" value="{{$post->title}}">
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    @php
                                    $listCategoryId = $post->categories->pluck('id')->toArray();
                                    @endphp
                                    <select class="form-control{{ $errors->has('category_id') ? ' is-invalid' : '' }}" name="category_id" id="category" data-toggle="select">
                                        <option value=""></option>
                                        @foreach($categories as $cate)
                                        <option @if(in_array($cate->id, $listCategoryId)) selected @endif value="{{$cate->id}}">{{$cate->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control{{ $errors->has('status') ? ' is-invalid' : '' }}" name="status" id="status" data-toggle="select">
                                        ...
                                        <option data-value2="{{$status['value2']}}" @if($post->status_text == $status['name']) selected @endif value="{{$status['value']}}">{{$status['name']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input class="form-control{{ $errors->has('published_at') ? ' is-invalid' : '' }}" name="published_at" id="published_at" type="datetime-local" value="{{$post->published_at}}" id="example-datetime-local-input">
                                </div>
                        ...
                        <div class="row">
                            @php
                            $listJobCategoryId = $post->jobCategories->pluck('id')->toArray();
                            @endphp
                            @foreach($hastags as $tag)
                            <div class="col-md-2 col-6">
                                <div class="custom-control custom-checkbox mb-3">
                                    <input class="custom-control-input" @if(is_array($listJobCategoryId) && in_array($tag->id,$listJobCategoryId))
                                    checked="true"
                                    @endif
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea style="height:150px" resize="none" class="form-control{{ $errors->has('summary') ? ' is-invalid' : '' }}" id="summary" name="summary">{{$post->summary}}</textarea>
                                </div>
                        ...
                        <div class="d-flex justify-content-between m-auto" style="width:320px">
                            ...
                            @if(auth()->user()->id == $post->creator_id && $post->status == \App\Enums\Post\EPostStatus::Draft)
                            ...
                            @else
                            <button type="submit" style="border-radius:20px; width:150px" class="ml-3 btn-block m-auto btn btn-primary">{{__('post.buttons.submit')}}</button>
                            @endif
                        </div>
                        @if(auth()->user()->id == $post->creator_id && $post->status == \App\Enums\Post\EPostStatus::Draft)
                        <div style="width:320px" class="mx-auto mt-3">
                            <button type="submit" style="border-radius:20px;" class="ml-3 btn-block m-auto btn btn-primary">{{__('post.buttons.submit')}}</button>
                        </div>
                        @endif
                    </form>
                </div>
    ...
    Dropzone.options.myDropzone = {
        ...
        init: function() {
            ...
            this.on("error", function(file, response) {
                if (response) {
                    // alert(response.message || 'Upload fail');
